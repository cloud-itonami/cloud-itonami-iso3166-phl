(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether a Philippines-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Philippines-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-ph-entity?
                                       true` (near-universal for Philippines
                                       ...
                                       Grounded in Philippine legal entity
                                       registration typically required for
                                       PhilGEPS awards (contracting authorities
                                       / GPPB).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(SEC/統一資格/PhilGEPS登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- ph-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-ph-entity? true`, INDEPENDENTLY verify
  `:has-ph-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-ph-entity?` ground truth (most Philippines public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-ph-entity? e))
                 (not (true? (:has-ph-entity? e))))
        [{:rule :ph-entity-missing
          :detail (str subject " はフィリピン居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- sec-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-sec? true`, INDEPENDENTLY check
  `:sec-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-sec? e))
                 (not (true? (:sec-verified? e))))
        [{:rule :sec-unverified
          :detail (str subject " はSEC確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (ph-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (sec-unverified-violations request st)
