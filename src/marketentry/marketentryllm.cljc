(ns marketentry.marketentryllm
  ...
  the PHL public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-ph-entity?=" (:has-ph-entity? e)
                        " sec-verified?=" (:sec-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-ph-entity? e))
                              (:has-ph-entity? e))
                          (or (not (:requires-sec? e))
                              (:sec-verified? e)))
                   0.9 0.3)}))
