...
2. Register the client's intake: business type, target public function,
   prior filing history in the Philippines if any.
3. Run the advisor in read-only mode against PhilGEPS (Philippine Government Electronic Procurement System).
...
- named referral relationship with Philippine-licensed counsel or a
...
a working referral relationship with Philippine-licensed counsel or a
registered agent for whatever licensed representation the law of
the Philippines requires for actual public-procurement filings.
