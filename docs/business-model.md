# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — the Philippines

## Classification

- Repository: `cloud-itonami-iso3166-phl`
- ISO 3166: `PHL` (the Philippines)
...
- Social impact: [:filipino-supplier-preference-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Philippine
  public contract
...
- registration walkthrough for PhilGEPS (Philippine Government Electronic Procurement System)
- business/tax registration checklist: a DTI (Department of Trade and Industry) business-name registration for sole proprietors, or SEC (Securities and Exchange Commission) registration for corporations; a Tax Identification Number (TIN) and VAT registration with the Bureau of Internal Revenue (BIR)
- local-content / preferential-procurement navigation: no blanket local-content quota, but the Government Procurement Reform Act includes preference provisions favoring Filipino suppliers and locally-manufactured goods on qualifying tenders
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Philippine-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in the Philippines).
